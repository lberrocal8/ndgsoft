...
const Facturas = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'POST') {
    const { ResoFacturas, FACTURA, Item, Referencia, DESCRIPCION, Observacion, CANTIDAD, VALUNITARIO, Descuento, VrFinal, SUBTOTAL, IVA, TOTAL, FECHA, CLIENTE, Usuario, Mesa, CodEMpleado, Nombre, Propina } = req.body;
    const nuevaFactura = await agregarFacturas(ResoFacturas, FACTURA, Item, Referencia, DESCRIPCION, Observacion, CANTIDAD, VALUNITARIO, Descuento, VrFinal, SUBTOTAL, IVA, TOTAL, FECHA, CLIENTE, Usuario, Mesa, CodEMpleado, Nombre, Propina);
    if (nuevaFactura) {
      return res.status(201).json({ message: 'Factura agregada correctamente' });
    } else {
...
async function agregarFacturas (ResoFacturas: string, FACTURA: string, Item: number, Referencia: string, DESCRIPCION: string, Observacion: string, CANTIDAD: number, VALUNITARIO: number, Descuento: number, VrFinal: number, SUBTOTAL: number, IVA: number, TOTAL: number, FECHA: Date, CLIENTE: string, Usuario: string, Mesa: string, CodEMpleado: string, Nombre: string, Propina: number) {
  try {
