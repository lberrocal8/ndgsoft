...
export default async function getProducts (req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await pool.request().query('SELECT ROW_NUMBER() OVER (ORDER BY [Descripcion]) AS idProducto, Referencia, Descripcion, VrVenta, Existencia FROM Mercancia;');
    await pool.close();
